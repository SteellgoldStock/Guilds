...
public class PlayerManager {
    ...
    public static HashMap<UUID, Session> session;
    ...
    public static void newPlayer(Connection connection, Player player) {
        try {
            final PreparedStatement statement = connection.prepareStatement("INSERT INTO players (uuid, name) VALUES (?, ?)");
    ...
    public static boolean loadSession(Connection connection, Player player) {
        try {
            final PreparedStatement statement = connection.prepareStatement("SELECT * FROM players WHERE uuid = ?");
            ...
            if (resultSet.next()) {
                ...
                PlayerManager.session.put(player.getUniqueId(), session);
                return true;
            } else {
                return false;
            }
    ...
    public static Session getSession(Player player) {
        return PlayerManager.session.get(player.getUniqueId());
    }

    public static boolean isSessionOpen(Player player) {
        return PlayerManager.session.containsKey(player.getUniqueId());
    }

    public static void closeSession(Player player) {
        // Envoyer les données de la session dans la base de données (Requete UPDATE)
        final Session session = PlayerManager.session.get(player.getUniqueId());
        ...
        Guild.getInstance().getServer().getScheduler().runTaskAsynchronously(Guild.getInstance(), () -> {
            try {
                ...
                PlayerManager.session.remove(player.getUniqueId());
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
